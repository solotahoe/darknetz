...
import ProductList from "./scotia.json";
import {
...
export default function Scotiabank() {
  const [Bank, setBank] = useState("SCOTIA BANK");
            ...
            <tbody>
              {paginatedPost.map((item) => (
                <tr>
                  ...
                  <td>
                    <Link
                      to={`/invoice/scotia/${item.buy}`}
                      state={{ from: "scotia", amount: `${item.buy}` }}
                    >
