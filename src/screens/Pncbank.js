...
import ProductList from "./pnc.json";
import {
...
export default function Pncbank() {
  const [Bank, setBank] = useState("PNC BANK");
            ...
            <tbody>
              {paginatedPost.map((item) => (
                <tr>
                  ...
                  <td>
                    <Link
                      to={`/invoice/pnc/${item.buy}`}
                      state={{ from: "pnc", amount: `${item.buy}` }}
                    >
