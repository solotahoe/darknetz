...
import ProductList from "./rbc.json";
import {
...
export default function Rbcbank() {
  const [Bank, setBank] = useState("RBC BANK");
            ...
            <tbody>
              {paginatedPost.map((item) => (
                <tr>
                  ...
                  <td>
                    <Link
                      to={`/invoice/rbc/${item.buy}`}
                      state={{ from: "rbc", amount: `${item.buy}` }}
                    >
