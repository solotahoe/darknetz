...
import ProductList from "./huntington.json";
import {
...
export default function Huntingtonbank() {
  const [Bank, setBank] = useState("HUNTINGTON");
            ...
            <tbody>
              {paginatedPost.map((item) => (
                <tr>
                  ...
                  <td>
                    <Link
                      to={`/invoice/Huntingtonbank/${item.buy}`}
                      state={{ from: "Huntingtonbank", amount: `${item.buy}` }}
                    >
